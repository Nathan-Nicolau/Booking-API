error id: 1FA472066B72587FBDB147E2FF4DC8A8
file:///C:/Users/Gamer/Documents/Teste/Booking-API/booking-api/app/controllers/ReservaController.scala
### java.lang.AssertionError: assertion failed: bad position: [412:410]
...
class ReservaController @Inject()(val cc: ControllerComponents, val reservaService: ReservaService)(implicit ec: ExecutionContext) extends AbstractController(cc) {
  ...
  def getTodosQuartos() = Action.async {
    reservaService.listarTodos().map { rooms =>
      Ok(rooms.mkString(", "))
    }
  }
  ...
  //GET adicionar-quarto-inventario
  def adicionarQuartoInventario(novoQuarto: NovoQuartoInventario) = Action.async(parse.json) { request =>
    
    request.body.validate[NovoQuartoInventari@@o].fold(
      errors => Future.successful(BadRequest(Json.obj("Erro"))),
      periodo => if (!periodo.datasPeriodoValidas()) {
        Future.successful(BadRequest(Json.obj("error" -> "Datas inválidas")))
      } else {
        // aqui chamaria seu service
        Future.successful(Ok(Json.obj("message" -> "Reserva criada", "periodo" -> periodo)))
      }
    )

    reservaService.adicionarQuartoInventario(novoQuarto).map { retorno =>
      Ok(retorno.mensagem.toString())
    }
  }

  //POST remover-quarto-inventario
  ...
  def getReservasPeriodo(periodo: ReservaPeriodo) = Action.async {
    reservaService.getReservasPeriodo(periodo).map { retorno =>
      Ok(retorno.toString())
     }
  }
